...
# ---- Run bootstrap ----
options(future.globals.maxSize = 6 * 1024^3)  # 6 GB
icc_boot_results_linear <- bootstrap_icc_safe(data_for_boot, R = 5000)

save(icc_boot_results_linear, file = "./markdown/analysis/processed_data/bootstrap_icc_linear_simulation.Rdata")

# ---- Summarize bootstrap CI ----
ci_summary_icc_simulation_linear <- icc_boot_results_linear %>%
  group_by(bootstrap_id, approach, component, weight, penalty) %>% 
  summarize(missing = mean(missing), 
            cor = mean(cor),
            icc = mean(icc)) %>% 
