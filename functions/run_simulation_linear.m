function[] = run_simulation_linear(n_simuls, sliced_trials, time_vec, method_table, db_location)
    ...
    mean_shift = max(normrnd(method_table.simulation_stretch, method_table.simulation_sd, 1, n_subjects), 0); % Draw this from normal distribution for all participants
    shift_sd = method_table.stretch_sd;
    rng 'shuffle'
    parfor isimul = 1:n_simuls
        ...
        for isubject = 1:n_subjects
            ...
            % a_param set to 1
            simul_mat(isubject, :) = interpolate_shifted_template(time_vec, mean(simul_trials, 3, "omitnan"), normrnd(mean_shift(isubject), shift_sd, 1, 1));
        end
        ...
        % Protect against NaN
        protected_mean_shift = mean_shift';
        protected_mean_shift(isnan(empirical_shift)) = [];
        empirical_shift(isnan(empirical_shift)) = [];
    
        average_shift = mean(protected_mean_shift);
        sd_shift = sqrt(var(protected_mean_shift));
        computed_icc = compute_icc([protected_mean_shift, empirical_shift]');
        simul_overall(isimul, :) = [method_table.task_id, method_table.method_id, simul_id, average_shift, sd_shift, computed_icc];
        
        % Write into raw data
        simul_table = convert_result_matrix_to_table(results_simul, method_table.task_id, method_table.method_id, simul_id, ones(1, n_subjects), mean_shift, ["b_param", "latency", "fit_cor", "fit_distance"], 1:n_subjects);
    ...
    for isubject = 1:n_subjects
        ...
        shifts(isnan(shifts)) = [];
        mean_subject_shift(isubject) = mean(shifts);
        sd_subject_shift(isubject) = sqrt(var(shifts));
    end
    
    subject_mat = [repelem(method_table.task_id, n_subjects); repelem(method_table.method_id, n_subjects); 1:n_subjects; mean_shift; mean_subject_shift; sd_subject_shift]';
